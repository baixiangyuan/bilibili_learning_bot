...
if not exist "Data\config.json" (
    if exist "config.example.json" (
        echo [INIT] Copying config.example.json to Data\config.json ...
        copy /y "config.example.json" "Data\config.json" >nul
    ) else (
        echo [WARN] config.example.json missing. Create Data\config.json manually.
    )
)
...
set "WEB_PORT=8080"
set "BILI_ACCOUNT_DATA_DIR=Data"
set "BILI_ACCOUNT_NAME=Account1"
